...
Firebase AuthenticationのGoogleログイン（`GoogleAuthProvider`）は「Sign in with Google」の
基本スコープ（email/profile/openid）のみを使うため、Google Cloud ConsoleのOAuth同意画面が
テストモードであっても、テストユーザー登録の有無に関わらず一般のGoogleアカウントでも
ログイン自体は通ってしまう（Googleの仕様。テストユーザー制限はこの基本スコープには実質適用されない）。
そのため、アプリ側で許可リストを実装している。

- `src/lib/allowList.ts`: `VITE_ALLOWED_EMAILS`（`.env.local`からカンマ区切り）を解析し、
  メールアドレスが許可リストに含まれるか判定する。**未設定の場合は誰でもログイン可能**（開発時の事故防止のため、空リストでの全拒否ではなく全許可にフォールバックする仕様）
- `useAuth.ts`: `onAuthStateChanged` のコールバック内で許可リストチェックを行う。
  ログイン直後だけでなく、ページ再読み込みによるセッション復帰時にも毎回判定される。
  許可リストから誰かを削除した場合は次回のページ読み込みで弾かれる。
- 許可リスト外のアカウントでログインを試みた場合は即 `signOut` し、`authError` にエラー設定。
  `LoginPage` にエラー表示される。
- Firestoreセキュリティルール側にも同様の許可リストを設定し、コード側をすり抜けて直接
  Firestoreにアクセスしようとした場合でも、サーバー側でブロックされる（`firestore.rules.md` 参照）。
  **メールアドレスを追加・削除する際は `.env.local` とFirestoreルールの両方を更新する必要がある**
...
リポジトリには含まれていない（Firebase Console側で直接管理）。現在運用中のルールには、
通常のuid制限に加えて `request.auth.token.email` によるログイン許可リスト判定が入っている。
詳細・最新の更新案は `firestore.rules.md` を参照。
...
`VITE_ALLOWED_EMAILS` はログインを許可するGoogleアカウントのメールアドレス（カンマ区切り）。
未設定の場合は制限なし（誰でもログイン可能）になるので、本番運用では必ず設定すること。
追加・削除時はFirestoreルール側（`firestore.rules.md`）も合わせて更新する。

GitHub ActionsのSecretsにも同じ7つを登録済み。
...
- [x] ログイン許可リスト（`VITE_ALLOWED_EMAILS` + Firestoreルールの二重チェック）
